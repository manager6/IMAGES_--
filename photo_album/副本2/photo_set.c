...
PI *pro_init()
{
    ...
    pro_inf->mmap_init_point = mmap(NULL, pro_inf->screen_size * LCD_PIXEL_BYTES, PROT_READ | PROT_WRITE, MAP_SHARED, pro_inf->lcd, 0);
    if (pro_inf->mmap_init_point == MAP_FAILED)
    {
    ...
    pro_inf->thumbnail_displayed = false;
...
int display_image(PI *pro_inf, IMAGE *image_inf, char *file_path)
{
    ...
    for (int y = 0, n = 0; y < pro_inf->LCD_H; y++)
    {
        for (int x = 0; x < pro_inf->LCD_W; x++, n += 3)
        {
            ...
            {
                *(pro_inf->mmap_init_point + (pro_inf->LCD_W * (pro_inf->LCD_H - 1 - y) + x)) =
...
int touch_event_handler(PI *pro_inf, int current_mode)
{
    ...
    if (pro_inf->touch.type == EV_KEY && pro_inf->touch.code == BTN_TOUCH && pro_inf->touch.value == 0)
    {
        ...
        case MAIN_MODE:
            ...
            if (pro_inf->TOUCH_X > 65 && pro_inf->TOUCH_X < 260 && pro_inf->TOUCH_Y > 345 && pro_inf->TOUCH_Y < 390)
            {
...
int select_interface(PI *pro_inf, IMAGE *image_inf)
{
    ...
    while (1)
    {
        ...
        case EXIT_SYSTEM_MODE:
            ...
            exit(0);
        default:
            pro_inf->thumbnail_displayed = false; // 其他模式下重置标志位
            break;
        }
...
int loading_interface(PI *pro_inf, int progress)
{
    ...
    for (int y = 400; y < 450; y++)
    {
        for (int x = 100; x < 700; x++)
        {
            if (x < (progress * 8))
            {
                *(pro_inf->mmap_init_point + 800 * y + x) = 0x000ff000;
            }
            else
            {
                *(pro_inf->mmap_init_point + 800 * y + x) = 0;
            }
        }
    }
    return 0;
...
void cleanup(PI *pro_inf, IMAGE *image_inf)
{
    ...
    {
        if (pro_inf->mmap_init_point != MAP_FAILED)
            munmap(pro_inf->mmap_init_point, pro_inf->screen_size * LCD_PIXEL_BYTES);
        if (pro_inf->lcd != -1)
            close(pro_inf->lcd);
        if (pro_inf->touch_lcd != -1)
            close(pro_inf->touch_lcd);
